...
def euler_err(η,quad):
    ...
    for i_k in range(len(k_grid)):  # Iterate over k and z grids
        ...
        for i_z in range(len(z_grid)):
            ...
            z       = z_grid[i_z]
            c       = c_poly(k,z,η)
            l       = ((1/(chi*c))*(1 - α)*z*(k**α))**(nu/(1 + α*nu))
            k_prime = z * (k**α) * (l**(1-α)) + (1-δ) * k - c
            ...
            for i_q in range(len(q_nodes)):
                ...
                l_prime = ((1/(chi*c_prime))*(1 - α)*z_prime*(k_prime**α))**(nu/(1 + α*nu))
                
                # E += q_weights[i_q] * β * c_prime**(-γ) * (α * z_prime * (k_prime**(α-1)) * (l_prime**(1 - α))  + (1-δ))            
                E += q_weights[i_q] * β * (c_prime/c)**(-γ) * (α * z_prime * (k_prime**(α-1)) * (l_prime**(1 - α))  + (1-δ))            
...
# Plot consumption policy function approximation
...
# Plot labor policy function approximation
l       = ((1/(chi*c_poly(kg,zg,η_opt)))*(1 - α)*zg*(kg**α))**(nu/(1 + α*nu))
fig = plt.figure(figsize=(12,9))
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(kg,
                zg,
...
# c_cheb_approx = c_cheb_vec(kg,zg,gamma_star,k_low,k_high,z_low,z_high,p_k,p_z)
# l_cheb       = ((1/(chi*c_cheb_approx))*(1 - α)*zg*(kg**α))**(nu/(1 + α*nu))

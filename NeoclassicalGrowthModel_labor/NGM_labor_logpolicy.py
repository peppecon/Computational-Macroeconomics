...
class NGM_logpolicy(Parameters):
    ...
    def labor_policy(self,c,z,k):
        
        chi = self.chi
        nu = self.nu
        alpha = self.alpha
        
        labor = (((1 - alpha)/(chi*c))*z*(k**alpha))**(nu/(1 + alpha*nu))
    ...
    def Euler_error(self,gamma):
        ...
        cons_policy = self.approx_consumption_policy(gamma,grid_z_full,grid_k_full/k_ss)
        ...
        for node in range(len(q_nodes)):
            ...
            cons_next = self.approx_consumption_policy(gamma,z_prime,k_prime/k_ss)
...
ax.plot_surface(kg,
                ...
                l_star,
                rstride=2, cstride=2,
                edgecolor='red', lw=0.8, alpha=0)
ax.set_xlabel('k', fontsize=14)
ax.set_ylabel('z', fontsize=14)
ax.set_title('Labor Policy Function',fontsize=subtitle_font)
plt.savefig('latex_problem_set/cl_logpolicy.png')
plt.show()
